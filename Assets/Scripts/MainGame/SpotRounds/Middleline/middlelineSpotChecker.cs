...
public class middlelineSpotChecker : MonoBehaviour
{
    private middlelineSpotCalling _spotcall;
    public Text[] Ticket;
    ...
    void Start()
    {
        ...
        checkspotno = "no";
        _spotcall = GameObject.FindWithTag("GameHandling").GetComponent<middlelineSpotCalling>();
        _nakama = GameObject.FindWithTag("MainMenuManager").GetComponent<NakamaTest>();
    }

    public IEnumerator checkSpot()
    {
        StartCoroutine(Enablespotcheck("MiddleLine"));
    ...
    {
        var createuser_url = "http://34.121.136.31/housiekings/CheckmiddlelineWinners.php";
