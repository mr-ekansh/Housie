...
public class toplineSpotChecker : MonoBehaviour
{
    private toplineSpotCalling _spotcall;
    public Text[] Ticket;
    ...
    void Start()
    {
        ...
        checkspotno = "no";
        _spotcall = GameObject.FindWithTag("GameHandling").GetComponent<toplineSpotCalling>();
        _nakama = GameObject.FindWithTag("MainMenuManager").GetComponent<NakamaTest>();
    }

    public IEnumerator checkSpot()
    {
        StartCoroutine(Enablespotcheck("TopLine"));
    ...
    {
        var createuser_url = "http://34.121.136.31/housiekings/ChecktoplineWinners.php";
