...
public class bottomlineSpotChecker : MonoBehaviour
{
    private bottomlineSpotCalling _spotcall;
    public Text[] Ticket;
    ...
    void Start()
    {
        ...
        checkspotno = "no";
        _spotcall = GameObject.FindWithTag("GameHandling").GetComponent<bottomlineSpotCalling>();
        _nakama = GameObject.FindWithTag("MainMenuManager").GetComponent<NakamaTest>();
    }

    public IEnumerator checkSpot()
    {
        StartCoroutine(Enablespotcheck("BottomLine"));
    ...
    {
        var createuser_url = "http://34.121.136.31/housiekings/CheckbottomlineWinners.php";
